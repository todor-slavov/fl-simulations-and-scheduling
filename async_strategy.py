...
class AsynchronousStrategy:
    ...
    def average(self, global_parameters: Parameters, model_update_parameters: Parameters, t_diff: float, num_samples: int, server_age: int) -> Parameters:
        """Compute the average of the global and client parameters."""
        if self.async_aggregation_strategy == "fedasync":
            return self.weighted_average_fedasync(global_parameters, model_update_parameters, t_diff, num_samples, server_age)
    ...
    def weighted_average_fedasync(self, global_parameters: Parameters, model_update_parameters: Parameters, t_diff: float, num_samples: int, server_age: int) -> Parameters:
        ...
        return ndarrays_to_parameters(self.aggregate_fedasync(parameters_to_ndarrays(global_parameters), parameters_to_ndarrays(model_update_parameters), t_diff, num_samples=num_samples, server_age=server_age))

    def aggregate_fedasync(self, global_param_arr: NDArrays, model_update_param_arr: NDArrays, t_diff: float, num_samples: int, server_age: int) -> NDArrays:
        """Compute weighted average with the formula params_new = (1-alpha) * params_old + alpha * (model_update_params)"""
        # Calculate the total number of examples used during training
        alpha_coeff = self.fedasync_mixing_alpha # / ((1 + 0.025) ** (server_age))
        # 12.48
