...
class FlowerClient(fl.client.NumPyClient):
    ...
    def __init__(self, trainloader, vallodaer, num_classes, cpu: bool = True, cpu_vs_gpu: bool = False) -> None:
        super().__init__()
        ...
        if not cpu_vs_gpu:
            ...
            if(torch.cuda.is_available() and cpu is False):
    ...
    def fit(self, parameters, config):
        ...
        # print("nananna")
        lr = config["lr"]
        lr = lr * (1 / (1 + 0.05) ** config["server_age"])
        print(config["server_age"])
        ...
        return self.get_parameters({}), len(self.trainloader), {"t_diff": end_time - start_time, "server_age": config["server_age"]}
...
def generate_client_fn(trainloaders, valloaders, num_classes):
    ...
    def client_fn(cid: str):
        ...
        fl_client.set_cid(cid)
        return fl_client

    # return the function to spawn client
    return client_fn

def generate_client_fn_cpu_gpu(trainloaders, valloaders, num_classes):
    """Return a function that can be used by the VirtualClientEngine.

    to spawn a FlowerClient with client id `cid`.
    """

    def client_fn(cid: str, cpu: bool):
        # This function will be called internally by the VirtualClientEngine
        # Each time the cid-th client is told to participate in the FL
        # simulation (whether it is for doing fit() or evaluate())
